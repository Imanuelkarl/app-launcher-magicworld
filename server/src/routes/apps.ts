...
const payload = (body: Record<string, unknown>) =>
  Object.fromEntries(fields.filter((k) => k in body).map((k) => [k, body[k]]));
router.get("/", async (req, res, next) => {
  try {
    ...
    res.json(await App.find(query).sort({ isFeatured: -1, publishedAt: -1 }));
  } catch (e) {
    next(e);
  }
});
...
router.put(
  ...
  async (req, res, next) => {
    try {
      const current = await App.findById(req.params.id);
      if (!current)
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Editors can only modify their own uploads." });
      res.json(
        await App.findByIdAndUpdate(
          req.params.id,
          { ...payload(req.body), updatedBy: req.user._id },
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const item = await App.findByIdAndDelete(req.params.id);
      if (!item)
        return res.status(404).json({ message: "Application not found" });
      res.status(204).end();
    } catch (e) {
